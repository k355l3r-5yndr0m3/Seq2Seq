...
def save_check_point(model: nn.Module, optimizer: optim.Optimizer, scheduler: optim.lr_scheduler.LRScheduler, save_path: str = "checkpoint.pth"):
    state_dict = {
...
def load_check_point(model: nn.Module, optimizer: optim.Optimizer, scheduler: optim.lr_scheduler.LRScheduler, load_path: str = "checkpoint.pth"):
...
def keep_n_checkpoints(model: nn.Module, optimizer: optim.Optimizer, scheduler: optim.lr_scheduler.LRScheduler,
                       checkpoints_dir: str = "checkpoints/", keep_n: int = 4):
    ...
    if newest_idx is not None:
        ...
        save_check_point(model, optimizer, scheduler, save_path=path.join(checkpoints_dir, f"checkpoint-{newest_idx+1}.pth"))
    else:
        save_check_point(model, optimizer, scheduler, save_path=path.join(checkpoints_dir, "checkpoint-0.pth"))
